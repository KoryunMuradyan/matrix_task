...
Matrix<T>::Matrix(int& arg_rows, int& arg_cols)
	...
	, cols_(arg_cols)
{
	std::vector<std::vector<T>> raw_matrix_(rows_, std::vector<T>(cols_,
								  T(NULL)));
}
...
void Matrix<T>::gaussianEliminate()
{
	...
	std::for_each(this->raw_matrix_.begin(), this->raw_matrix_.end(), 
		...
		[&row_size, &tmp_mltmap] (std::vector<T> i) {
			auto zero_num = std::find_if(i.begin(), i.end(), 
							      Not_Zero);
			int tmp = int(zero_num - i.begin());
			if (tmp == row_size) {
				tmp = -1;
			}
			tmp_mltmap.insert(std::pair<int,
			std::vector<int>>(tmp, i));
		}; );
	auto it = this->raw_matrix_.begin();
	std::for_each(tmp_mltmap.begin(), tmp_mltmap.end(), [&](auto i) {
		*it++ = i.second;}
        );
